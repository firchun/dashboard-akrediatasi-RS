      ...
      <form method="POST" action="{{ route('settings.update') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-[600px]">
          <div class="mb-3.5">
            ...
            <input type="text" name="hospital_name" value="{{ $setting->hospital_name }}"
          ...
          <div class="mb-3.5">
            ...
            <input type="date" name="target_date"
              value="{{ $setting->target_date ? $setting->target_date->format('Y-m-d') : '' }}"
              class="w-full px-3 py-2 border border-line rounded-lg text-sm bg-white focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/12" />
          </div>
        </div>
        ...
        <div class="flex gap-5 flex-wrap mb-4 items-center">
          <label
            class="chk cursor-pointer flex items-center gap-2 font-semibold text-xs text-ink bg-white border border-line rounded-lg px-3 py-2">
            <input type="checkbox" name="is_pendidikan" value="1" {{ $setting->is_pendidikan ? 'checked' : '' }}
              class="w-4 h-4 accent-teal" /> RS Pendidikan (16 bab)
          </label>
          <label
            class="chk cursor-pointer flex items-center gap-2 font-semibold text-xs text-ink bg-white border border-line rounded-lg px-3 py-2">
            <input type="checkbox" name="prognas_full" value="1" {{ $setting->prognas_full ? 'checked' : '' }}
              class="w-4 h-4 accent-teal" /> PROGNAS wajib 100%
          </label>
          <label class="inline-flex items-center gap-2 text-xs font-semibold text-ink">
            Mode hitung:
            <select name="calc_mode"
              class="px-2.5 py-1.5 border border-line rounded-lg text-xs bg-white focus:outline-none focus:border-teal">
