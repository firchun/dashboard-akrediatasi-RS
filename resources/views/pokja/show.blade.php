...
@section('content')
  <main class="wrap pt-6">
    <div class="mb-4">
      <a href="{{ route('pokja.index') }}" class="text-xs font-semibold text-teal hover:text-teal-deep">← Kembali ke
        Pokja</a>
    </div>
        ...
        <tbody>
          @forelse($pokja->regulasis as $reg)
            <tr>
              ...
              <td>
                <span
                  class="inline-block text-[10.5px] font-bold font-mono px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-slate-600">
                  {{ $reg->jenis }}
                </span>
              </td>
              ...
              <td>
                @if(!empty($reg->link))
                  <a href="{{ $reg->link }}" target="_blank" rel="noopener"
          ...
          @empty
            <tr class="empty-row">
              <td colspan="6" class="text-center text-slate-400 italic">Belum ada regulasi.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
    ...
    @if($pokja->epItems->count() > 0)
      ...
      <div class="board p-4 mb-5 shadow-custom bg-card border border-line rounded-[14px]">
        <table class="eptbl">
          <thead>
            <tr>
              <th class="w-[124px]">Standar / No. Urut</th>
              <th>Uraian</th>
          ...
          <tbody>
            @foreach($pokja->standars as $std)
              <tr class="bg-slate-50 border-t-2 border-slate-200">
                <td class="font-mono font-bold text-xs text-ink pt-3 pb-3 align-top">{{ $std->kode }}</td>
                <td colspan="4" class="text-xs font-bold text-slate-700 leading-normal pt-3 pb-3">{{ $std->uraian }}</td>
              </tr>
              @foreach($std->epItems as $ei)
                <tr class="border-b border-line-soft">
                  <td class="font-mono font-bold text-xs text-ink pt-2 pb-2 pl-4 text-center">{{ $ei->no_urut }}</td>
                  <td class="text-xs text-slate-700 leading-normal pt-2 pb-2">{{ $ei->uraian }}</td>
                  <td class="pt-2 pb-2">
                    @php $buktid = []; @endphp
                    @foreach($BUKTI as $bk) @if($ei->{'bukti_' . strtolower($bk)}) @php $buktid[] = $bk; @endphp @endif
                    @endforeach
                    <span
                      class="font-mono text-[11px] font-bold text-slate-700 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded-md">
                      {{ implode('', $buktid) ?: '—' }}
                    </span>
                  </td>
                  <td class="pt-2 pb-2">
                    <span class="chip"
                      style="background:{{ $ei->nilai === 'TL' ? 'var(--st-selesai-bg)' : ($ei->nilai === 'TS' ? 'var(--st-proses-bg)' : ($ei->nilai === 'TT' ? 'var(--danger-bg)' : ($ei->nilai === 'TDD' ? 'var(--st-belum-bg)' : '#eef3f4'))) }};color:{{ $ei->nilai === 'TL' ? 'var(--st-selesai)' : ($ei->nilai === 'TS' ? 'var(--st-proses)' : ($ei->nilai === 'TT' ? 'var(--danger)' : ($ei->nilai === 'TDD' ? 'var(--st-belum)' : 'var(--muted-2)'))) }}">
                      {{ $ei->nilai ?: '—' }}
                    </span>
                  </td>
                  <td class="text-xs font-semibold text-slate-600 pt-2 pb-2">{{ $ei->pic ?: '—' }}</td>
                </tr>
              @endforeach
            @endforeach
          </tbody>
    ...
    <footer class="mt-8 pt-4 border-t border-line text-slate-400 text-xs">
      <span>Standar acuan: STARKES Kemenkes (KMK HK.01.07/MENKES/1596/2024) | BY Firmansyah diana</span>
    </footer>
  </main>
@endsection