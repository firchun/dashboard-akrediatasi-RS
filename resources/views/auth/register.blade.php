...
<div class="max-w-[480px] mx-auto my-10 px-5">
  <div class="bg-card border border-line rounded-[14px] p-7 shadow-custom">
    ...
    <form method="POST" action="{{ route('register.post') }}">
      ...
      <div class="mb-4">
        ...
        <select name="pokja_id" required
          ...
          @foreach($pokjas as $p)
            <option value="{{ $p->id }}" {{ old('pokja_id') == $p->id ? 'selected' : '' }}>[{{ $p->code }}] {{ $p->name }} — {{ $p->group }}</option>
          @endforeach
        </select>
      </div>
    ...
    <div class="mt-4.5 pt-4 border-t border-line-soft text-center">
      ...
      <a href="{{ route('login') }}" class="inline-block mt-1.5 text-sm font-semibold text-teal hover:text-teal-deep">Masuk</a>
    </div>
  </div>
</div>
@endsection
